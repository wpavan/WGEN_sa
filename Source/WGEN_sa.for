...
!  Modified to run as a stand-alone.
C-----------------------------------------------------------------------
C  REVISION HISTORY
...
!  07/16/2012 CHP Modified to work as a stand-alone routine.
C-----------------------------------------------------------------------
c  Called by:   WEATHR
...
!      INTEGER MULTI, RUN, REPNO
     ...
     &  TMAX,TMIN,RAIN,XLAT,XLONG,XPRV(3)   !,TDEW
      REAL TAV, TAMP, RHUM, XELEV
...
!-----------------------------------------------------------------------
        IF (MEWTH .EQ. 'W') THEN
          CALL WGENIN(
     &        FILEW, PATHWT,                              !Input
     &        CF, INSI, TAMP, TAV, XELEV, XLAT, XLONG, Y) !Output
...
!        READ (LINE,'(6X,2F9.0)',IOSTAT=ERRNUM) XLAT,XLONG
        READ (LINE,'(2X,A4,2F9.0,3F6.0)',IOSTAT=ERRNUM) 
     &            INSI, XLAT, XLONG, XELEV, TAV, TAMP
